...
class HttpTransport:
    ...
    def _process_queue(self):
        ...
        while True:
            try:
                data = self.span_queue.get(timeout=0.1)
                try:
                    requests.post(f"{self.url}/spans", json=data, headers=headers, timeout=2)
                except Exception:
                    pass
            ...
            try:
                ...
                try:
                    requests.post(f"{self.url}/v1/logs", json=log, headers=headers, timeout=2)
                except Exception:
                    pass
