...
@Component("manualAnalysisAlertHandler")
public class ManualAnalysisHandler implements AlertHandler {
    ...
    public void handle(PaymentAlertEvent alert) {
        logger.info("Enviando email para equipe de análise manual: {} | Pagamento: {}",
                alert.getAlertType(), alert.getPaymentId());
    }

    @Override
    public AlertType getSupportedType() {
        return AlertType.MANUAL_ANALYSIS;
    }
}
